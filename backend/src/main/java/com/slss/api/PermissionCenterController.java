...
public class PermissionCenterController {
  ...
  public Page<AuditLog> permissionAudit(@RequestParam(defaultValue="") String action,@PageableDefault(size=50,sort="createdAt",direction=Sort.Direction.DESC) Pageable pageable){return action.isBlank()?auditLogs.findAll(pageable):auditLogs.findByActionContainingIgnoreCase(action,pageable);}
