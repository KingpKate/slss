...
public class PermissionCenterController {
  ...
  public List<?> permissionAudit(){return auditLogs.findAll(Sort.by(Sort.Direction.DESC,"createdAt")).stream().filter(x->x.getAction().toUpperCase(Locale.ROOT).contains("PERMISSION")||x.getTargetType().toUpperCase(Locale.ROOT).contains("PERMISSION")||x.getTargetType().equals("SCOPE")).limit(200).toList();}
