...
public class ScanTableController {
 ...
 @GetMapping("/templates/page") @Transactional public Map<String,Object> listTemplatesPage(@RequestParam(defaultValue="0") int page,@RequestParam(defaultValue="20") int size){var pageable=PageRequest.of(Math.max(0,page),Math.min(Math.max(size,1),100),Sort.by(Sort.Direction.ASC,"customerName","model"));var ids=tenantScope.currentTenantIds();var p=tenantScope.isSystemAdmin()?templates.findByActiveTrue(pageable):(ids.isEmpty()?org.springframework.data.domain.Page.<ScanTemplate>empty(pageable):templates.findByActiveTrueAndTenant_IdIn(ids,pageable));var content=p.getContent().stream().map(this::templateDto).toList();return pageDto(content,p.getNumber(),p.getSize(),p.getTotalElements(),p.getTotalPages());}
