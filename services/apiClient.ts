...
export const api = {
  ...
  permissionAudit: (page = 0, size = 20, action = '') => request<{content:any[];totalElements:number;totalPages:number}>(`/permission-center/audit?page=${page}&size=${size}${action ? `&action=${encodeURIComponent(action)}` : ''}`),
  ...
  loginBackgroundAssets: () => request<Array<{ id: number; url: string; mimeType: string; width: number; height: number; sizeBytes: number }>>('/settings/branding/backgrounds'),
  uploadLoginBackground: (file: File) => { const body = new FormData(); body.append('file', file); return request<{ id: number; url: string }>('/settings/branding/backgrounds', { method: 'POST', body, headers: {} }); },
  deleteLoginBackground: (id: number) => request<void>(`/settings/branding/backgrounds/${id}`, { method: 'DELETE' }),
  ...
  createAiChannel: (payload: any) => request<any>('/settings/ai/channels', { method: 'POST', body: JSON.stringify(payload) }),
  updateAiChannel: (id: number, payload: any) => request<any>(`/settings/ai/channels/${id}`, { method: 'PUT', body: JSON.stringify(payload) }),
