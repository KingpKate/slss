...
import { Search, Activity, Box, FileText, Truck, CheckCircle, AlertTriangle, ChevronRight, Server } from 'lucide-react';
...
const Portal: React.FC = () => {
  ...
  const handleSearch = async (e: React.FormFormEvent) => {
    ...
    try {
      // Search for order by SN
      const orderRes = await fetch(`/api/orders?search=${encodeURIComponent(sn.trim())}`);
      if (!orderRes.ok) throw new Error('查询失败');
      const orders = await orderRes.json();
      const order = Array.isArray(orders)
        ? orders.find((o: any) => o.machine_sn?.toLowerCase() === sn.trim().toLowerCase())
        : null;

      if (!order) {
        setError('未找到该序列号的维修记录。请检查 SN 是否正确。');
        setLoading(false);
        return;
      }

      // Fetch lifecycle, tests, logistics in parallel
      const [lifecycleRes, logisticsRes] = await Promise.all([
        fetch(`/api/lifecycle?machine_sn=${encodeURIComponent(sn.trim())}`).catch(() => null),
        fetch(`/api/orders/${order.id}`).catch(() => null),
      ]);

      const lifecycle = lifecycleRes?.ok ? await lifecycleRes.json() : [];
      const orderDetail = logisticsRes?.ok ? await logisticsRes.json() : order;

      setResult({
        order,
        lifecycle: Array.isArray(lifecycle) ? lifecycle : [],
        tests: [], // Test reports API not yet implemented
      ...
      <div className="bg-slate-900 text-white py-16 px-4 text-center relative overflow-hidden">
        <div className="relative z-10">
          ...
          <form onSubmit={handleSearch} className="mt-10 max-w-lg mx-auto flex gap-2 shadow-2xl rounded-lg p-1 bg-white/10 backdrop-blur-sm border border-white/20">
            ...
            />
            <button type="submit" disabled={loading} className="bg-blue-600 px-8 py-4 rounded-md font-bold hover:bg-blue-500 transition flex items-center text-white disabled:opacity-50">
