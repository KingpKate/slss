...
const Portal: React.FC = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    api.portalAsset(sn.trim()).then((data:any) => { const order = data.orders?.[0]; if (!order) throw new Error('not-found'); setResult({order, lifecycle:data.lifecycle || [], tests:data.tests || [], logistics:data.logistics || [], reports:data.reports || []}); setError(''); }).catch(() => { setResult(null); setError('未找到该序列号的维修记录。'); });
  };
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans">
      ...
      <footer className="py-8 bg-white border-t border-gray-200 text-center text-gray-400 text-sm">
        <div className="max-w-7xl mx-auto px-4">
           <p className="mb-2">&copy; 2023 SLSS V2.1 生产售后一体化服务平台</p>
