...
const AdminPanel: React.FC = () => {
  ...
  const saveAiChannel = async () => {
    try {
      ...
      const saved = aiChannelForm.id ? await api.updateAiChannel(aiChannelForm.id, payload) : await api.createAiChannel(payload);
  ...
  const editAiChannel = (channel: any) => setAiChannelForm({ id: channel.id, version: channel.version || 0, name: channel.name || '', provider: channel.provider || 'custom', protocol: channel.protocol || 'OPENAI_COMPATIBLE', baseUrl: channel.baseUrl || '', model: channel.model || '', apiKey: '', enabled: channel.enabled !== false, priority: channel.priority ?? 100, weight: channel.weight ?? 100, timeoutMs: channel.timeoutMs ?? 30000 });
  const resetAiChannelForm = () => setAiChannelForm({ id: null, version: 0, name: '', provider: 'custom', protocol: 'OPENAI_COMPATIBLE', baseUrl: '', model: '', apiKey: '', enabled: true, priority: 100, weight: 100, timeoutMs: 30000 });
