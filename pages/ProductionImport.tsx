...
const generateDefaultColumns = (): ColumnGroup[] => [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'mem_sns', label: 'SN', type: 'input', widthClass: 'w-28' },
  ...
  },
  {
    ...
    colorClass: 'gray',
    fields: [
      { key: 'pcie_info', label: '信息', type: 'input', widthClass: 'w-20' },
...
// Optimized Wrapper to avoid inline arrow functions in render map
...
const GridSelect = React.memo(({ 
  ...
  return (
    <select 
       className="w-full h-full border-none bg-transparent outline-none text-xs text-gray-600 focus:bg-white text-center cursor-pointer appearance-none hover:bg-gray-50" 
       value={value || ''} 
       onChange={handleChange}
    >
...
const ImportWizardModal: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  onImport: (result: ImportWizardResult) => void;
  ...
  const uploadFileToServer = async (file: File): Promise<{
    filename: string;
    originalFilename: string;
    url: string;
    uploadedAt: string;
    ...
    reader.onload = async (e) => {
      try {
        ...
        const mappedRows: GridRow[] = jsonData.map((row: any) => ({
            ...
            pcie_info: getVal(row, ['PCIE信息', 'pcie_info']),
            pcie_sn: getVal(row, ['PCIE SN', 'pcie_sn', 'pcie']),
        }));

        // Step 2: Upload original file to server
        ...
        setError(err.message || "文件处理失败，请检查格式");
      } finally {
        setLoading(false);
      }
    };
...
const ColumnSettingsModal: React.FC<{
  ...
  onClose: () => void;
  columns: ColumnGroup[];
  onAdd: (type: ColumnType, count: number) => void;
  ...
  const typeMap: Record<string, string> = {
    'mem': '内存 (Memory)', 'hdd': '硬盘 (HDD)', 'psu': '电源 (PSU)', 'pcie': 'PCIE', 'other': '其他 (Other)'
  };

  const getCount = (type: string) => counts[type] || 1;
  const setCount = (type: string, val: number) => setCounts(prev => ({...prev, [type]: Math.max(1, val)}));
    ...
    <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <div className="mb-6">
            <h4 className="text-sm font-bold text-gray-700 mb-3">添加列 / 增加 SN 列</h4>
              ...
              {Object.entries(typeMap).map(([type, label]) => (
                <div key={type} className="flex items-center gap-2">
                   ...
                   <input 
                      ...
                      value={getCount(type)}
                      onChange={e => setCount(type, parseInt(e.target.value))}
                   />
                   <button 
                      onClick={() => onAdd(type as ColumnType, getCount(type))}
                      className="px-3 py-1.5 bg-blue-50 text-blue-700 border border-blue-200 rounded text-sm hover:bg-blue-100 flex items-center font-bold"
                   >
                      <Plus className="w-3 h-3 mr-1"/> 增加 {getCount(type)} 列
                   </button>
                </div>
              ))}
              <div className="flex items-center gap-2 pt-2 border-t mt-2">
                 <div className="flex-1 text-sm text-gray-700 font-medium">CPU</div>
                 <button onClick={() => onAdd('cpu', 1)} className="px-3 py-1.5 bg-purple-50 text-purple-700 border border-purple-200 rounded text-sm hover:bg-purple-100 font-bold ml-auto flex items-center"><Plus className="w-3 h-3 mr-1"/> 添加 CPU 组</button>
              </div>
            </div>
          </div>
                ...
                <div key={col.id} className="flex justify-between items-center p-2 border border-gray-200 rounded bg-gray-50">
                  <div className="flex items-center">
                    ...
                    <div>
                       ...
                       <div className="text-xs text-gray-400 truncate max-w-[200px]">{col.fields.map(f => f.label).join(', ')}</div>
                    </div>
                  </div>
                  {col.type !== 'mb' && (
...
const EntryGrid: React.FC<{ 
  contractNo: string;
  initialRows?: GridRow[];
  fileInfo?: { filename: string; originalFilename: string; url: string; uploadedAt: string };
  onBack: () => void;
}> = ({ contractNo, initialRows, fileInfo, onBack }) => {
  const { user } = useAuth();
  ...
  const addColumnGroup = (type: ColumnType, count: number = 1) => {
    setColumns(prev => {
        const newCols = [...prev];
        const groupIndex = newCols.findIndex(c => c.type === type);

        // Special handling for CPU: CPU usually implies adding a whole new group (CPU 2, CPU 3), not just SNs to CPU 1
        if (type === 'cpu') {
            const existingCpus = newCols.filter(c => c.type === 'cpu').length;
            const index = existingCpus + 1;
            const newGroup: ColumnGroup = { 
                ...
                colorClass: 'purple', 
                fields: [
                    { key: `cpu_model_${index}`, label: '型号', type: 'input', widthClass: 'w-16' }, 
                    { key: `cpu_sn_${index}`, label: 'SN', type: 'input', widthClass: 'w-24' },
                    { key: `cpu_operator_${index}`, label: 'OP', type: 'select', widthClass: 'w-16' }
                ] 
            };
            // Insert after last CPU
            let lastCpuIdx = -1;
            newCols.forEach((c, i) => { if(c.type === 'cpu') lastCpuIdx = i; });
            if (lastCpuIdx !== -1) newCols.splice(lastCpuIdx + 1, 0, newGroup);
            else newCols.push(newGroup);
            return newCols;
        }

        // General Logic for Mem, HDD, PSU, PCIE: Merge into existing group
        if (groupIndex !== -1) {
            const group = { ...newCols[groupIndex] };
            const fields = [...group.fields];

            // Determine where to insert: before the OP/Operator column
            const opIndex = fields.findIndex(f => f.key.includes('operator') || f.key.includes('OP'));
            const insertIndex = opIndex === -1 ? fields.length : opIndex;

            // Generate new fields
            const newFields: ColumnField[] = [];
            for(let i = 0; i < count; i++) {
                const uniqueSuffix = `${Date.now()}_${i}`;
                newFields.push({
                    key: `${type}_sn_ext_${uniqueSuffix}`,
                    label: 'SN', // Keep label simple to save space
                    type: 'input',
                    widthClass: 'w-24'
                });
            }
            ...
            // Insert fields
            fields.splice(insertIndex, 0, ...newFields);
            group.fields = fields;
            newCols[groupIndex] = group;
            return newCols;
        } 
        
        // Fallback: Group doesn't exist, create default structure
        let newGroup: ColumnGroup;
        const id = `${type}_${Date.now()}`;
        const baseFields = [
            { key: `${type}_info_new`, label: '信息', type: 'input' as const, widthClass: 'w-20' },
            { key: `${type}_sn_new`, label: 'SN', type: 'input' as const, widthClass: 'w-24' },
            { key: `${type}_operator_new`, label: 'OP', type: 'select' as const, widthClass: 'w-16' }
        ];
        
        // Add extra SNs if count > 1
        if (count > 1) {
             const extras = [];
             for(let i=1; i<count; i++) {
                 extras.push({ key: `${type}_sn_new_${i}`, label: 'SN', type: 'input' as const, widthClass: 'w-24' });
             }
             // Insert extras between SN and OP
             baseFields.splice(2, 0, ...extras);
        }

        switch(type) {
            case 'mem': newGroup = { id, type, title: '内存 (Mem)', colorClass: 'green', fields: baseFields }; break;
            case 'hdd': newGroup = { id, type, title: '硬盘 (HDD)', colorClass: 'orange', fields: baseFields }; break;
            case 'psu': newGroup = { id, type, title: '电源 (PSU)', colorClass: 'yellow', fields: baseFields }; break;
            case 'pcie': newGroup = { id, type, title: 'PCIE 扩展卡', colorClass: 'gray', fields: baseFields }; break;
            default: newGroup = { id, type, title: '其他', colorClass: 'gray', fields: baseFields };
        }

        // Insert at logical position
        let insertPos = newCols.length;
        const priority = TYPE_PRIORITY[type] || 99;
        const nextGroupIndex = newCols.findIndex(c => (TYPE_PRIORITY[c.type] || 99) > priority);
        if (nextGroupIndex !== -1) insertPos = nextGroupIndex;
        
        newCols.splice(insertPos, 0, newGroup);
        return newCols;
    });
  };
                    ...
                    {rows.map((row, idx) => (
                        <tr key={row._id} className={`${idx === 0 ? 'bg-yellow-50/60 border-b-2 border-yellow-100 shadow-inner' : 'hover:bg-blue-50/30'} group transition-colors`}>
                            ...
                            <td className="p-0 border-r">
                                <GridCell id={row._id} field="customer_name" value={row.customer_name} onChange={handleCellChange} className="w-full h-full px-2 py-2 outline-none text-xs transition-colors text-center bg-transparent" />
                            </td>
                            <td className="p-0 border-r">
                                <GridCell id={row._id} field="model" value={row.model} onChange={handleCellChange} className="w-full h-full px-2 py-2 outline-none text-xs transition-colors text-center bg-transparent" />
                            </td>
                            <td className="p-0 border-r">
                                <GridCell id={row._id} field="machine_sn" value={row.machine_sn} onChange={handleCellChange} placeholder="Scan SN" className={`w-full h-full px-2 py-2 outline-none text-xs transition-colors text-center ${getValidationClass(row.machine_sn)}`} />
                            </td>
                            {columns.map(col => (col.fields.map(field => (
                                <td key={`${col.id}_${field.key}`} className={`p-0 border-r ${field.type === 'select' ? 'bg-gray-50/50' : ''}`}>
                                    {field.type === 'select' ? 
                                        <GridSelect id={row._id} field={field.key} value={row[field.key]} onChange={handleCellChange} options={operators} /> : 
                                        <GridCell id={row._id} field={field.key} value={row[field.key]} onChange={handleCellChange} className={`w-full h-full px-2 py-2 outline-none text-xs transition-colors text-center bg-transparent ${getValidationClass(row[field.key])}`} />
                                    }
                                </td>
                            ))))}
    ...
    <div className="h-[calc(100vh-6rem)]">
      {view === 'dashboard' && (
        <ContractDashboard 
          ...
          onImport={(result) => {
            const { rows, fileInfo } = result;
            setSelectedContract(rows[0].contract_no);
            setInitialRows(rows);
            setImportedFileInfo(fileInfo);
