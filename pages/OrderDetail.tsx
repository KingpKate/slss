...
import { analyzeFault, AnalysisResult } from '../services/aiService';
...
const OrderDetail: React.FC = () => {
  ...
  const [operationModal,setOperationModal]=useState<'part'|'test'|'logistics'|null>(null);
  const [operationForm,setOperationForm]=useState<Record<string,string>>({});
  const [extensionRows,setExtensionRows]=useState<{parts:any[];tests:any[];logistics:any[]}>({parts:[],tests:[],logistics:[]});
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
          const o = await afterSalesApi.getOrder(Number(id)) as RepairOrder;
          if (cancelled || sequence !== loadSequence.current) return;
          setOrder(o);
          setReceivedConfigText(o.received_config_json || o.shipment_config_json || '{}');
          const [parts,tests,logistics,report]=await Promise.all([
            afterSalesApi.parts(Number(id)),
            afterSalesApi.tests(Number(id)),
            afterSalesApi.logistics(Number(id)),
            afterSalesApi.report(Number(id)).catch(()=>null)
          ]);
          if (cancelled || sequence !== loadSequence.current) return;
          setExtensionRows({parts,tests,logistics});
          if (report) {
            const { diagnosis, resolution, testConclusion } = report as any;
            setOrder(prev => prev ? { ...prev, diagnosis, resolution, testConclusion } as RepairOrder : prev);
          }
      }
      catch (e: any) { if (!cancelled && sequence === loadSequence.current) setSaveStatus(e?.message || '工单加载失败'); }
      finally { if (!cancelled && sequence === loadSequence.current) setLoading(false); }
    };
    load();
  ...
  const handleSaveOrder = (updates?: Partial<RepairOrder>) => {
    ...
    const action = updates?.status && updates.status !== order.status
        ? afterSalesApi.transition(order.id, updates.status, '工单详情更新')
        : afterSalesApi.updateOrder(order.id, {customerName:updates?.customer_name, faultDescription:updates?.fault_description, actualFaultDescription:updates?.actual_fault_description ?? order.actual_fault_description, machineSn:updates?.machine_sn, receivedConfigJson: receivedConfigText || updates?.received_config_json});
      action.then((saved:any) => { setOrder({...order,...updates,...saved}); setSaveStatus('工单已保存!'); setIsEditingReceived(false); }).catch((e:any) => setSaveStatus(e.message));
  };
  ...
  // Suspend or cancel an order; backend requires a non-blank reason for both.
  const handleSuspendOrCancel = async (target: OrderStatus) => {
    if (!order) return;
    const label = target === OrderStatus.SUSPENDED ? '挂起' : '取消';
    const reason = window.prompt(`请输入${label}原因（必填）`, '');
    if (!reason || !reason.trim()) { setSaveStatus(`${label}原因不能为空`); return; }
    try {
      const saved: any = await afterSalesApi.transition(order.id, target, reason.trim());
      setOrder({ ...order, status: (saved && saved.to) || target });
      setSaveStatus(`工单已${label}`);
      setTimeout(() => setSaveStatus(''), 3000);
    } catch (e: any) {
      setSaveStatus(e?.message || `${label}失败`);
    }
  };

  const handleSaveReport = (reportData: any) => {
    if (!order) return;
    afterSalesApi.saveReport(order.id,{diagnosis:reportData.diagnosis || reportData.faultCause || '待补充',resolution:reportData.resolution || reportData.solution || '待补充',testConclusion:reportData.testConclusion || ''}).then(()=>{setSaveStatus('维修报告已保存');setShowReport(false);}).catch((e:any)=>setSaveStatus(e.message));
  };
  const downloadSignedReport = async () => {
    if (!order) return;
    try { const {token}=await afterSalesApi.reportDownloadToken(order.id); const base=window.location.pathname.split('/').slice(0,2).join('/'); window.open(`${base}/api/v1/service-orders/${order.id}/report/download?token=${encodeURIComponent(token)}`,'_blank'); } catch(e:any){setSaveStatus(e.message||'报告下载失败');}
  };

  const submitOperation=async()=>{
    if(!order||!operationModal)return;
    try{
      if(operationModal==='part')await afterSalesApi.addPart(order.id,{partName:operationForm.partName,oldSn:operationForm.oldSn,newSn:operationForm.newSn});
      if(operationModal==='test')await afterSalesApi.addTest(order.id,{testType:operationForm.testType,result:operationForm.result,details:operationForm.details});
      if(operationModal==='logistics')await afterSalesApi.addLogistics(order.id,{direction:operationForm.direction,carrier:operationForm.carrier,trackingNumber:operationForm.trackingNumber,notes:operationForm.notes});
      const [parts,tests,logistics]=await Promise.all([afterSalesApi.parts(order.id),afterSalesApi.tests(order.id),afterSalesApi.logistics(order.id)]);
      setExtensionRows({parts,tests,logistics});setOperationModal(null);setOperationForm({});setSaveStatus('操作记录已保存');
    }catch(e:any){setSaveStatus(e.message||'保存失败');}
  };
  ...
  const runAIAnalysis = async () => {
    ...
    try {
      ...
      setSaveStatus(e?.message || 'AI 分析失败');
    } finally {
  ...
  const getCurrentStepIndex = () => {
    ...
    if (order.status === OrderStatus.CANCELLED) return 5; // Cancelled (terminal)
    if (order.status === OrderStatus.SHIPPED) return 4; // Logistics
    if (order.status === OrderStatus.SUSPENDED) return 0; // Paused (keep at start)
  ...
  // Suspend/Cancel availability per OrderWorkflow (backend contract)
  const canSuspend = [OrderStatus.ASSIGNED, OrderStatus.CHECKING, OrderStatus.QA_AGING, OrderStatus.SHIPPED].includes(order.status);
  const canCancel = [OrderStatus.PENDING, OrderStatus.ASSIGNED, OrderStatus.CHECKING, OrderStatus.QA_AGING, OrderStatus.SUSPENDED].includes(order.status);
  const canResume = order.status === OrderStatus.SUSPENDED;
  ...
  return (
    <div className="space-y-6 pb-12">
         ...
         <div className="flex space-x-2 items-center">
            ...
            {order.status === OrderStatus.QA_AGING && (
              <>
                 ...
                 {order.report_data_json && <button onClick={downloadSignedReport} className="ml-2 rounded bg-slate-800 px-3 py-2 text-sm font-medium text-white hover:bg-slate-700">签名下载报告</button>}
            ...
            {canSuspend && (
               <button
                  onClick={() => handleSuspendOrCancel(OrderStatus.SUSPENDED)}
                  className="flex items-center px-4 py-2 bg-amber-500 text-white rounded shadow hover:bg-amber-600 text-sm font-medium"
               >
                  <AlertTriangle className="w-4 h-4 mr-2" /> 挂起
               </button>
            )}
            {canResume && (
               <button
                  onClick={() => updateStatus(OrderStatus.ASSIGNED, '工单已恢复处理')}
                  className="flex items-center px-4 py-2 bg-emerald-600 text-white rounded shadow hover:bg-emerald-700 text-sm font-medium"
               >
                  <RefreshCw className="w-4 h-4 mr-2" /> 恢复处理
               </button>
            )}
            {canCancel && (
               <button
                  onClick={() => handleSuspendOrCancel(OrderStatus.CANCELLED)}
                  className="flex items-center px-4 py-2 bg-red-600 text-white rounded shadow hover:bg-red-700 text-sm font-medium"
               >
                  <X className="w-4 h-4 mr-2" /> 取消工单
               </button>
            )}
         </div>
      </div>
      ...
      <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-bold text-gray-800">维修执行记录</h3><p className="text-xs text-gray-500 mt-1">配件更换会同步资产组件和生命周期；测试与物流记录直接持久化。</p></div>
        <div className="flex gap-2"><button onClick={()=>{setOperationForm({});setOperationModal('part')}} className="px-3 py-2 bg-blue-600 text-white rounded text-sm flex items-center"><Package className="w-4 h-4 mr-1"/>配件更换</button><button onClick={()=>{setOperationForm({});setOperationModal('test')}} className="px-3 py-2 bg-indigo-600 text-white rounded text-sm flex items-center"><ClipboardCheck className="w-4 h-4 mr-1"/>新增测试</button><button onClick={()=>{setOperationForm({direction:'OUTBOUND'});setOperationModal('logistics')}} className="px-3 py-2 bg-orange-500 text-white rounded text-sm flex items-center"><Truck className="w-4 h-4 mr-1"/>物流登记</button></div></div>
        <div className="grid md:grid-cols-3 gap-3 mt-4 text-xs"><div className="bg-blue-50 rounded p-3">配件记录：{extensionRows.parts.length}</div><div className="bg-indigo-50 rounded p-3">测试记录：{extensionRows.tests.length}</div><div className="bg-orange-50 rounded p-3">物流记录：{extensionRows.logistics.length}</div></div>
      </div>
      ...
      {operationModal && <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"><div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
        <div className="flex justify-between items-center mb-5"><h3 className="text-lg font-bold">{operationModal==='part'?'配件更换':operationModal==='test'?'测试记录':'物流登记'}</h3><button onClick={()=>setOperationModal(null)}><X/></button></div>
        <div className="space-y-3">
          {operationModal==='part'&&<><input className="w-full border rounded p-2" placeholder="配件名称 *" value={operationForm.partName||''} onChange={e=>setOperationForm({...operationForm,partName:e.target.value})}/><input className="w-full border rounded p-2" placeholder="旧配件 SN *" value={operationForm.oldSn||''} onChange={e=>setOperationForm({...operationForm,oldSn:e.target.value})}/><input className="w-full border rounded p-2" placeholder="新配件 SN *" value={operationForm.newSn||''} onChange={e=>setOperationForm({...operationForm,newSn:e.target.value})}/></>}
          {operationModal==='test'&&<><input className="w-full border rounded p-2" placeholder="测试类型 *" value={operationForm.testType||''} onChange={e=>setOperationForm({...operationForm,testType:e.target.value})}/><select className="w-full border rounded p-2" value={operationForm.result||''} onChange={e=>setOperationForm({...operationForm,result:e.target.value})}><option value="">选择结果 *</option><option value="PASS">通过</option><option value="FAIL">失败</option></select><textarea className="w-full border rounded p-2" placeholder="测试明细" value={operationForm.details||''} onChange={e=>setOperationForm({...operationForm,details:e.target.value})}/></>}
          {operationModal==='logistics'&&<><select className="w-full border rounded p-2" value={operationForm.direction||'OUTBOUND'} onChange={e=>setOperationForm({...operationForm,direction:e.target.value})}><option value="INBOUND">寄入</option><option value="OUTBOUND">寄出</option></select><input className="w-full border rounded p-2" placeholder="承运商 *" value={operationForm.carrier||''} onChange={e=>setOperationForm({...operationForm,carrier:e.target.value})}/><input className="w-full border rounded p-2" placeholder="运单号 *" value={operationForm.trackingNumber||''} onChange={e=>setOperationForm({...operationForm,trackingNumber:e.target.value})}/><textarea className="w-full border rounded p-2" placeholder="备注" value={operationForm.notes||''} onChange={e=>setOperationForm({...operationForm,notes:e.target.value})}/></>}
        </div>
        <div className="flex justify-end gap-2 mt-5"><button onClick={()=>setOperationModal(null)} className="px-4 py-2 border rounded">取消</button><button onClick={submitOperation} className="px-4 py-2 bg-blue-600 text-white rounded">确认保存</button></div>
      </div></div>}
    </div>
  );
};
