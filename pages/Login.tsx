...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    ...
    api.branding().then((value: Branding) => {
      if (!active) return;
      const next = { ...fallbackBranding, ...value, backgroundImages: value.backgroundImages || [] };
      setBranding(next);
  ...
  return <main className="slss-login-page relative grid min-h-screen place-items-center overflow-hidden p-4" style={{ background: solidBackground }}>
    ...
    {backgrounds.length > 0 && <div className="absolute inset-0" style={{ backgroundColor: `rgba(2,6,23,${Math.min(0.9, Math.max(0, branding.backgroundOverlay ?? 0.58))})` }} />}
