...
const ServiceOrders: React.FC = () => {
  ...
  useEffect(() => {
    afterSalesApi.listOrders().then(data => { setOrders(data as RepairOrder[]); setLoadError(null); }).catch(err => setLoadError(err?.message || '加载售后工单失败'));
  }, []);
  ...
  const handleCreateOrder = () => {
    ...
    afterSalesApi.createOrder({ orderNumber: `RMA-${Date.now()}`, customerName: formData.customer_name, faultDescription: formData.fault_description, machineSn: formData.machine_sn })
      .then((created: any) => navigate(`/orders/${created.id}`))
      .catch(err => setLoadError(err?.message || '创建工单失败'));
    setShowModal(false);
  };
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, `ServiceOrders_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
        try {
            ...
            const importedRows = jsonData.map((row: any) => {
                // Basic validation
                ...
                return {
                    ...
                    trackingNumber: row['单号']
                };
            }).filter((o: any) => o !== null);

            if (importedRows.length > 0) {
              Promise.all(importedRows.map((row: any) => afterSalesApi.createOrder({
                orderNumber: `RMA-IMP-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
                customerName: row.customer_name,
                faultDescription: row.fault_description,
                machineSn: row.machine_sn,
                shipmentModel: row.shipment_model,
                shipmentConfigJson: row.shipment_config_json,
                actualFaultDescription: row.actual_fault_description,
                trackingNumber: row.trackingNumber,
              }))).then(async () => {
                const latest = await afterSalesApi.listOrders();
                setOrders(latest as RepairOrder[]);
                alert(`成功导入 ${importedRows.length} 条工单`);
              }).catch((error: any) => setLoadError(error?.message || '批量导入工单失败'));
            } else {
  ...
  const KANBAN_COLS = [
    ...
    { id: 'suspended', title: '已挂起', statuses: [OrderStatus.SUSPENDED], color: 'bg-amber-50 border-amber-200' },
    { id: 'cancelled', title: '已取消', statuses: [OrderStatus.CANCELLED], color: 'bg-red-50 border-red-200' },
  ];
